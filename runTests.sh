...
do
   ...
   do
      ...
      OUTPUT_PDF="${bases[i]}${n}.pdf"

      ./bin/tester -c ./bin/config.cfg --SHOW_IMG=0 --SAVE_IMG=0 --SAVE_CONFIG=0 --REF_IMG=${REFIMG} --MAT_IMG=${MATIMG} --HOMOG=${HOMOGRAPHY} && octave --silent --eval "plotter(\"PR.csv\",\"${TITLE}\",\"${OUTPUT_PDF}\")" && pdfcrop --margins 10 ${OUTPUT_PDF} ${OUTPUT_PDF}
   done
done
